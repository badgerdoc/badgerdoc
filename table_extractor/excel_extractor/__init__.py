...
from table_extractor.excel_extractor.extractor import clean_xlsx_images
...
CASCADE_CONFIG_PATH = (
    ...
    else Path(__file__).parent.parent.parent.joinpath(
...
CASCADE_MODEL_PATH = (
    ...
    else Path(__file__).parent.parent.parent.joinpath("models/epoch_20_w18.pth")
)
...
def match_inf_res(xlsx_path: Path, images_dir: Path):
    ...
    for page_num, worksheet in enumerate(workbook.worksheets):
        ...
        if not blocks:
            inf_tables, headers = cascade_rcnn_detector.inference_image(
            ...
            for inf_table in inf_tables:
                ...
                for row_id in range(1, last_row):
                    ...
                    if (
                        inf_table.bbox.top_left_y
                        < prev_coord * y_scale
                        < inf_table.bbox.bottom_right_y
                        and inf_table.bbox.top_left_y
                        < coord * y_scale
                        < inf_table.bbox.bottom_right_y
                    ):
                        ...
                        < prev_coord * y_scale
                        < inf_table.bbox.bottom_right_y
                        and not inf_table.bbox.top_left_y
                        < coord * y_scale
                        < inf_table.bbox.bottom_right_y
                    ):
                        if (
                        ...
                        if (coord * y_scale - inf_table.bbox.top_left_y) / (
                            coord * y_scale - prev_coord * y_scale
                        ) > 0.3:
                            rows_in_table.append(
                                (
                                    row_id,
                ...
                for col_id in range(1, last_col):
                    ...
                    if (
                        inf_table.bbox.top_left_x
                        < prev_coord * x_scale
                        < inf_table.bbox.bottom_right_x
                        and inf_table.bbox.top_left_x
                        < coord * x_scale
                        < inf_table.bbox.bottom_right_x
                    ):
                        cols_in_table.append(
                            (
                                col_id,
                                int(prev_coord * x_scale),
                                int(coord * x_scale),
                            )
                        )
                    elif (
                        ...
                        < prev_coord * x_scale
                        < inf_table.bbox.bottom_right_x
                        and not inf_table.bbox.top_left_x
                        < coord * x_scale
                        < inf_table.bbox.bottom_right_x
                    ):
                        if (
                        ...
                        if (coord * x_scale - inf_table.bbox.top_left_x) / (
                            coord * x_scale - prev_coord * x_scale
                        ) > 0.3:
                            cols_in_table.append(
                                (
                                    col_id,
                ...
                for m_range in worksheet.merged_cells.ranges:
                    if (
                        m_range.min_row in rows or m_range.max_row in rows
                    ) and (m_range.min_col in cols or m_range.max_col in cols):
                        rows = list(
                            range(
                                min(m_range.min_row, min(rows)),
                                max(m_range.max_row, max(rows)) + 1,
                            )
                        )
                        cols = list(
                            range(
                                min(m_range.min_col, min(cols)),
                                max(m_range.max_col, max(cols)) + 1,
                            )
                        )
                ...
                for t_col, (col, c_start, c_end) in enumerate(cols_in_table):
                    for t_row, (row, r_start, r_end) in enumerate(
                        rows_in_table
                    ):
                        in_merged = False
                        for m_range in m_ranges:
                            if (row, col) in list(m_range.cells):
                                in_merged = True
                                if (
                                    col == m_range.start_cell.col_idx
                                    and row == m_range.start_cell.row
                                ):
                                    cells.append(
                                        CellLinked(
                                            top_left_y=r_start,
                                            ...
                                            col=t_col,
                                            row=t_row,
                                            col_span=m_range.max_col
                                            - m_range.min_col
                                            + 1,
                                            row_span=m_range.max_row
                                            - m_range.min_row
                                            + 1,
                                            text_boxes=[
                                                TextField(
                                                    bbox=BorderBox(
                                                        ...
                                                        bottom_right_y=[
                                                            r_e
                                                            for r, r_s, r_e in rows_in_table
                                                            if r
                                                            == m_range.max_row
                                                        ][0]
                                                        if [
                                                        ...
                                                        else rows_in_table[-1][
                                                            2
                                                        ],
                                                        bottom_right_x=[
                                                            c_e
                                                            for c, c_s, c_e in cols_in_table
                                                            if c
                                                            == m_range.max_col
                                                        ][0]
                                                        if [
                ...
                struct_table_headered = get_headers_using_structured(
                    struct_table, headers
                )
                ...
                for col_id in range(1, last_col):
                    ...
                    if worksheet.cell(row_id, col_id).value and not any(
                        [
                            ...
                            for y1, x1, y2, x2 in table_zones
                        ]
                    ):
