...
class PaddleDetector:
    def __init__(self, det_model_dir: Path, cls_model_dir: Path):
        self.text_detector = get_text_detector(
            str(det_model_dir.absolute()), str(cls_model_dir.absolute())
        )

    def extract_table_text(
        self, img: numpy.ndarray, border_box: BorderBox
    ) -> List[TextField]:
        ...
        return [
            ...
            for cell in (
                BorderBox(b[0] + x1, b[1] + y1, b[2] + x1, b[3] + y1)
                for b in bboxes
            )
        ]
...
class PaddleSwitchWrapper(PaddleDetector):
    def __init__(self, det_model_dir: Path, cls_model_dir: Path, paddle_on):
        ...
        if paddle_on:
            super(PaddleSwitchWrapper, self).__init__(
    ...
    def extract_table_text(
        ...
        if not self.paddle_on:
            return []
        return super(PaddleSwitchWrapper, self).extract_table_text(
            img, border_box
        )
