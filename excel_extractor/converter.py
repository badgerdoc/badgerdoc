...
def should_skip_attr(
    cells: Dict[tuple, tuple], cell: Cell, another_cell: Cell, key, style
) -> bool:
    ...
    if style == "border":
        ...
        bottom = key == "bottom" and (
            cell.row == max(cells.keys())[1]
            or another_cell == max(cells.keys())[1]
        )
        left = key == "left" and (cell.column == 1 or another_cell.column == 1)
        right = key == "right" and (
            cell.row == max(cells.keys())[0]
            or another_cell == max(cells.keys())[0]
        )
...
def get_header_using_styles(
    tables: dict, styles_to_check: List = None, matched_counts: int = 1
) -> dict:
        ...
        for table in sheet_tables:
            ...
            for dims, colspan, rowspan, text, cell in table["cells"].values():
                ...
                for style in styles_to_check:
                    ...
                    if all(
                        [
                            getattr(cell_attr, key) == getattr(group_attr, key)
                            for key in dir(cell_attr)
                            if not should_skip_attr(
                                table["cells"],
                                cell,
                                groups[0][0][-1],
                                key,
                ...
                if (
                    len(styles_to_check) - matched_styles_count
                    <= matched_counts
                ):
                    groups[0].append((dims, colspan, rowspan, text, cell))
                else:
            ...
            if len([group for group in groups if group]) == 1:
                ...
                for group in groups:
                    if table["start"] in [
                        (cell[-1].column, cell[-1].row) for cell in group
                    ]:
                        table["headers"] = group
