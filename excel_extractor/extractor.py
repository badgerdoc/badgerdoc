...
class ExcelExtractor:
    ...
    def _check_interception(self, attr, table, cell) -> bool:
        index = int(attr == "row")
        return (
            table["end"][index] >= getattr(cell, attr) >= table["start"][index]
        )
    ...
    def iter_subtable_rows(self, sheet, first_cell: Cell, cell) -> Generator:
        max_col = (
            cell.column if sheet.max_column == cell.column else cell.column - 1
        )
        for row in sheet.iter_rows(
            min_row=first_cell.row, min_col=first_cell.column, max_col=max_col
        ):
            yield row
    ...
    def in_table(self, cell: Cell) -> bool:
        for table in self.ws_tables:
            if self._check_interception(
                "row", table, cell
            ) and self._check_interception("column", table, cell):
                return True
        return False

    def get_end_cell(
        self, sheet: Worksheet, first_cell: Cell, current_cell: Cell
    ) -> Cell:
        """
        Currently not used
        """
        ...
        column = (
            current_cell.column
            if current_cell.column == sheet.max_column
            else current_cell.column - 1
        )
        row_number -= 1
        return sheet.cell(row=row_number, column=column)

    def fill_cells(
        self, sheet: Worksheet, first_cell: Cell, last_cell: Cell
    ) -> dict:
        cells = {}
    ...
    def get_table(
        self, sheet: Worksheet, cell: Cell, table_started: Cell
    ) -> dict:
        """
        ...
        for row in sheet.iter_rows():
            ...
            for cell in row:
                ...
                cell_height = (
                    ...
                    if sheet.row_dimensions[cell.row].height
                    else 5.0
                )
                ...
                self.coordinates[(cell.column, cell.row)] = {
                    "top_left": (
                        x_counter + self.coordinates_offset_x,
                        y_counter + self.coordinates_offset_y,
                    ),
                    "top_right": (
                        x_counter + cell_width - self.coordinates_offset_x,
                        y_counter + self.coordinates_offset_y,
                    ),
                    "bottom_left": (
                        x_counter + self.coordinates_offset_x,
    ...
    def parse_sheet(self, sheet: Worksheet) -> list:
        ...
        self.finish_table(
            sheet, sheet[sheet.max_row][sheet.max_column - 1], sheet[1][0]
        )
